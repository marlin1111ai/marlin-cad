...
- `socket-tray-sampler.stl` — first Socket Tray coupon
  (`apps/web/src/lib/socketTrayGeometry.ts`), 240 x 60 x 24mm, one row of 5
  round blind pockets at stepped diameters (15, 18, 22, 25, 28mm OD, all
  20mm deep), left to right in ascending size. **Unvalidated**: these
  diameters and the pocket depth are the foreman's estimate for standard
  3/8"-drive sockets, not a measurement against real sockets — the coupon
  exists to be printed and checked against real sockets, not to be trusted
  as correct. See `reference/socket-tray-sampler-report.md` for the full
